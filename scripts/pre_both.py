...
# --- Run both variables ---
calc_df = process_variable("Calcification")
pp_df   = process_variable("Primary_Production")

# Merge on spatial–temporal coordinates and environmental variables
combined = pd.merge(
    calc_df,
    pp_df,
    on=["lat","lon","depth","time","temperature","sio4","po4","no3","o2","mld","DIC","TA","PAR"],
    how="outer"
)
print(f"Rows after combining Calcification and Primary Production: {len(combined)}")

# Save combined CSV
outfile = "/user/work/mv23682/Abil/studies/wiseman2024/data/calcif_pp_env_presample.csv"
